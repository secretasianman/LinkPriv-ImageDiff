...
def main():
    ...
    if arguments.file_size:
        try:
            size1 = os.path.getsize(arguments.file_name1)
        except os.error:
            print >> sys.stderr, "ERROR: Unable to access ", arguments.file_name1
            sys.exit(-1)
        try:
            size2 = os.path.getsize(arguments.file_name2)
        except os.error:
            print >> sys.stderr, "ERROR: Unable to access ", arguments.file_name2
        
        return float(abs(size1 - size2))/max(size1, size2)

    else:
        try:
            file1 = open(arguments.file_name1, 'r')
            string1 = file1.read()
        except IOError:
            print >> sys.stderr, "ERROR: Unable to open ", arguments.file_name1
        finally:
            file1.close()

        try:
            file2 = open(arguments.file_name2, 'r')
            string2 = file2.read()
        except IOError:
            print >> sys.stderr, "ERROR: Unable to open ", arguments.file_name2
        finally:
            file2.close()

        if arguments.levenshtein:
            try:
            ...
            print >> sys.stderr, """
                                 ...
                                 perhaps argparse isn't working.
                                 """
            sys.exit(-1)
