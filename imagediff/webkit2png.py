...
def main(website_url=None, output_file=None):
    ...
    parser.add_option("-g", "--geometry", dest="geometry", nargs=2, default=(1024, 768), type="int",
    ...
    def __main_qt():
        ...
        try:
            ...
            ## TODO
            db = MySQLdb.connect(host='kozmo.cis.upenn.edu', user='dki',
                                 passwd='2U1ZWVmZ', db='twitter03')
            cursor = db.cursor()
            cursor.execute('select expanded_link from twitter_links where expanded_link like "%?%" limit 20')
            update = db.cursor()
            for j in range(int(cursor.rowcount)):
                url = cursor.fetchone()[0]
                url_partition = url.partition('?')
                args = url_partition[2].split('&')

                arg_combinations = [arg_combination for i in range (len(args))
                                    for arg_combination in combinations(args, i)]

                # Create base image
                try:
                    out_file = open(path.join(path.realpath(path.dirname(__file__)),
                                              'media/base.png'), 'w')
                    renderer.render_to_file(url=url, file=out_file)
                    out_file.close()

                    # Image gen and comparison
                    min_hamming_distance = 1
                    min_url = url
                    for i, arg_combination in enumerate(arg_combinations):
                        ...
                        out_file = open(path.join(path.realpath(path.dirname(__file__)),
                                                  "media/img%d.png" % i), 'w')
                        renderer.render_to_file(url=url_combination, file=out_file)
                        out_file.close()

                        hamming_distance = imagediff('hamming',
                                                     'media/base.png',
                                                     "media/img%d.png" % i)
                        if hamming_distance < min_hamming_distance:
                            min_hamming_distance = hamming_distance
                            min_url = url_combination

                    update.execute("INSERT INTO processed_links (original, minimal, hamming_distance) VALUES ('%s', '%s', %f)" % (url, min_url, min_hamming_distance))
                except RuntimeError:
                    pass # Catch timeout
