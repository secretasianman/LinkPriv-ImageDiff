...
class HTMLMatcher(SequenceMatcher):
    ...
    # Added by kevinsu just to get a count of the differences
    def percentHtmlDiffs(self):
        count = 0
        total = 0
        for tag, i1, i2, j1, j1 in self.get_opcodes():
...
def percentDiffFiles(f1, f2):
    h = NoTagHTMLMatcher(open(f1).read(), open(f2).read())
    return h.percentHtmlDiffs()
...
if __name__ == '__main__':
    ...
    if not arguments.file1 or not arguments.file2:
